...
module.exports = {
  ...
  pluginDescription: 'Based on SWEXArtifact by Kiyyun. Logs the maximum possible efficiency for artifacts as they drop and are upgraded. Adapted from rune-drop-efficiency by Xzandro.',
  init(proxy) {
  ...
  logartifactDrop(artifact) {
	  try{
			...
			const artifactQuality = artifact.natural_rank;
			...
			const set_unit = artifact.unit_style=="" ? artifact.attribute : artifact.unit_style;
			const set_id = artifact.type * 10 + set_unit;
			const setTable = {
				11: 'Water',
        		12: 'Fire',
        		13: 'Wind',
			...
			return `<div class="rune item" style:"font-size=11px">
					  <div class="ui image ${color} label">
					  	<img src="../assets/artifacts/${setTable[set_id]}.png" />
					  	<div class ="set">${setTable[set_id]}</div>
  ...
  getArtifactTypeName(artifactType){
	...
	getArtifactSubStatAttributeName(attribute_id){
		...
		switch(attribute_id){
			...
			case 225:
				return "Counterattack/Co-op Attack DMG";
			case 226:
				return "ATK/DEF UP Effect";
			case 300:
	...
	getArtifactSubStatMaxAttribute(attribute_id){
		...
		switch(attribute_id){
			...
			case 224: //Single-target skill CRIT DMG X% on your turn
				return 20.0;
			case 225: //Counterattack/Co-op Attack DMG
				return 20.0;
			case 226: //ATK/DEF UP Effect
				return 25.0;
			...
			case 409: //Skill 3 Accuracy +${value}%
				return 30.0;
			case 410: //[Skill 3/4] CRIT DMG +X%
				return 30.0;
			case 411: //First Attack CRIT DMG +X%
				return 30.0; // max value possible
		}
		return 0;
	}
};
